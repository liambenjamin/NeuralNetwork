...
using FileIO, LinearAlgebra, HDF5, NPZ, JLD2, SparseArrays, Random, Colors, Main.Neurons, Main.Network, Main.loss, Main.penalty, Main.Helpers, Main.Partition, Main.LSTM
...
function train(ntwk, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag, partition, ntwk_type :: String)
    ...
    for i=1:num_epochs
		...
		#update_time = 0.0
			...
			error = LSTM.classification_error(ntwk, test_x, test_y, true)
			...
			epoch_file = "training-info/lstm/lstm_$(method)_$(seed).dat"
			...
			file_name = string("ntwks/lstm/lstm_$(method)_$(seed)_$(i).jld2")
...
hid_dim = 10
output_dim = 2
inp_dim = 50*100 + 2*hid_dim # corresponds to ntwk.features (input dim + 2*inital hidden state dim)
...
output_file = open(epoch_file, "a")
	printstyled(output_file, "**************\tTraining Method: LSTM $(method)\t**************\n\n", color = :yellow, bold = :true)
...
file_name = string("ntwks/lstm/lstm_$(method)_trained.jld2")
save(file_name, "ntwk", ntwk)
