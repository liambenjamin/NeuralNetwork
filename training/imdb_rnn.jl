...
using FileIO, LinearAlgebra, HDF5, NPZ, JLD2, SparseArrays, Random, Colors, Main.Neurons, Main.Network, Main.loss, Main.penalty, Main.Helpers, Main.Partition, Main.RNN
...
function train(ntwk, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag, partition, ntwk_type :: String)
    ...
    for i=1:num_epochs
		...
		#update_time = 0.0
			...
			error = RNN.classification_error(ntwk, test_x, test_y, true)
			...
			file_name = string("ntwks/rnn/rnn_$(method)_$(seed)_$(i).jld2")
...
hid_dim = 10
output_dim = 2
inp_dim = 50*100 + hid_dim # corresponds to ntwk.features (input dim + inital hidden state dim)
...
output_file = open(epoch_file, "a")
	printstyled(output_file, "**************\tTraining Method: RNN $(method)\t**************\n\n", color = :yellow, bold = :true)
...
file_name = string("ntwks/rnn/rnn_$(method)_trained.jld2")
save(file_name, "ntwk", ntwk)
