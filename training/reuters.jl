...
function train(ntwk, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag :: Bool, partition :: Vector{Vector{Int64}}, ntwk_type :: String, batch_size :: Int64, optimizer :: String, embedding :: Matrix)
	...
	# Load IMDB data
	train_x = npzread("reuters-params/keras-rnn-reuters-train-features.npz")["X"]
	test_x = npzread("reuters-params/keras-rnn-reuters-test-features.npz")["X"]
	# convert labels from Float32 to Float64
	train_y = convert.(Float64, npzread("reuters-params/keras-rnn-reuters-labels.npz")["y_train"])
	test_y = convert.(Float64, npzread("reuters-params/keras-rnn-reuters-labels.npz")["y_test"])
	...
	init_error = Helpers.reuters_test_error(ntwk, test_x, test_y, embedding, ntwk_type)
	...
	for i=1:num_epochs
		...
		for j=1:N#60,000 training samples
			...
			U, label = Helpers.embed(train_x_shuffle[j,:], embedding), train_y_shuffle[j,:]
		...
		error = Helpers.reuters_test_error(ntwk, test_x, test_y, embedding, ntwk_type)
		epoch_info["F"][i] = F / N
		epoch_info["G"][i] = G / N
		...
		file_name = string("../analysis/reuters/$(ntwk_type)_$(method)_$(i).jld2")
...
HID_DIM = 50
OUTPUT_DIM = 46
INP_DIM = 15000 # (150 × 100) --> 150 words embedded in R^(100)
FC_DIM = 0 # dimension of fully connect layer (optional)
SEQ_LENGTH = 150 # number of time steps in sequence
η = 1e-04 # learning rate default for adam (sgd = 0.01)
BATCH_SIZE = 32
NUM_EPOCHS = 5
LSMOD = loss.softmaxCrossEntropy # loss
PNMOD = penalty.var_phi # adjoint control function
OPT = "adam"
E = npzread("reuters-params/keras-rnn-reuters-embedding-matrix.npz")["E"]
...
KERAS_WEIGHTS = npzread("reuters-params/keras-rnn-reuters-adam.npz")
...
ϵ = 1e-6
neur1_params_k = npzread("reuters-params/keras-rnn-reuters-neur1-params.npz")["neur1_params"]
...
# load ntwk gradients
KERAS_GRADS = npzread("reuters-params/keras-rnn-reuters-adam-grad.npz")
...
label = test_y[1,:]
U = Helpers.embed(U,E)
U = vcat(collect(Iterators.flatten(U')), zeros(NTWK_NEW.hid_dim)) #flatten(150x100)+50
dP = paramGrad(LSMOD, NTWK_NEW, U, label, SYNCFLAG, PARTITION)

# verify gradients for first neuron
julia_grad_rec = dP[7451] # note: 7451 is max index in partition containing neuron 1
dP_neur1 = npzread("reuters-params/keras-rnn-reuters-neur1-grad.npz")["neur1_grad"]

# verify gradients for output neurons
julia_grad_out = dP[7501] # note: 7501 is first index in partition containing output (linear neuron)
...
# train ntwk
printstyled("Starting Training on Sequential Reuters Newswire Task:\t$(METHOD) -- $(OPT)\n\n", color=:yellow)
