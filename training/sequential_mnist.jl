...
using FileIO, NPZ, LinearAlgebra, CSV, JLD2, SparseArrays, MLDatasets, Random, Main.Neurons, Main.Network, Main.loss, Main.penalty, Main.Helpers, Main.Partition, Main.RNN, Main.LSTM, Main.GRU
...
function train(ntwk :: Network.network, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag :: Bool, partition :: Vector{Vector{Int64}}, ntwk_type :: String, batch_size :: Int64, optimizer :: String)
	...
	Helpers.mnist_init_error(ntwk, test_x, test_y, loss, ntwk_type)
	...
	for i=1:num_epochs
		...
		t = 0.0
		n_batches = length(train_y)
		for j=1:n_batches #iterate through batches
			t += @elapsed begin
				# updates network and returns batch summary (avg)
				b_F, b_G, b_Λ = Helpers.evaluate_batch(ntwk, train_x[j], train_y[j], loss, penalty, η, syncFlag, partition, optimizer, method, ntwk_type, m, v, i)
				F += b_F
				G += b_G
				Λ += b_Λ
			end # end elapsed
		...
		# returns test avg(s)
		test_F, test_G, test_Λ, test_error = Helpers.mnist_test_error(ntwk, test_x, test_y, loss, penalty, ntwk_type)
		...
		epoch_info["Λ_init"][i] = norm(Λ[λ_ind[1:ntwk.hid_dim]]) / n_batches # batch avg
		epoch_info["Λ_out"][i] = norm(Λ[λ_ind[end-ntwk.hid_dim+1:end]]) / n_batches # batch avg
		...
		file_name = string("../analysis/mnist3/$(ntwk_type)_$(method)_1e2_$(i).jld2")
...
# load ntwk weights
KERAS_WEIGHTS = npzread("mnist-params/keras-rnn-mnist-adam-four.npz")
...
# load ntwk gradients
KERAS_GRADS = npzread("mnist-params/keras-rnn-mnist-adam-grad-four.npz")
...
VERIFY_PAIR = npzread("mnist-params/keras-rnn-mnist-test-pair-four.npz")
label = VERIFY_PAIR["label"]
U = vcat(collect(Iterators.flatten(VERIFY_PAIR["U"][1,:,:]')), zeros(ntwk.hid_dim))
...
#training_output = train(ntwk, lsmod, pnmod, η, num_epochs, method, syncFlag, partition, ntwk_type, batch_size, opt)
#f = string("../analysis/mnist3/$(ntwk_type)_$(method)_1e2_output.jld2")
#save(f, "output", training_output)

f = string("../analysis/mnist3/$(ntwk_type)_partition.jld2")
