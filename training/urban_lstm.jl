...
using LinearAlgebra, Random, JLD, SparseArrays, NPZ, Random, Colors, Main.Neurons, Main.Network, Main.loss, Main.penalty, Main.Helpers, Main.Partition, Main.LSTM
...
function train(ntwk, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag :: Bool, partition :: Vector{Vector{Int64}}, ntwk_type :: String)
	...
	for i=1:num_epochs
		...
		#file_name = string("ntwks/lstm/lstm_$(method)_$(seed)_$(i).jld")
...
hid_dim = 5
output_dim = 10
inp_dim = 6960 + 2*hid_dim # corresponds to ntwk.features (input dim + inital hidden state dim)
...
#f = open(epoch_file, "a")
	...
	printstyled("\t\tInput Dim: $(inp_dim) ($(inp_dim-2*hid_dim) input + 2*$(hid_dim) init state)\n", color = :light_cyan)
...
#file_name = string("ntwks/lstm/lstm_$(method)_trained.jld")
#save(file_name, "ntwk", ntwk)
