	...
	for i=1:num_epochs
		...
		for j = 1:n_train
			...
			for k = 1:batch_size
				...
				U, label = vcat(rand(10), zeros(2*ntwk.hid_dim)), zeros(ntwk.results)
		...
		error, test_F = LSTM.add_test_error(ntwk, loss)
		...
		#file_name = string("toy_exp/lstm_epochs/ntwks/lstm_$(method)_$(info_layer)_$(seed)_$(i).jld2")
...
for i=1:length(seed)
	...
	#file_name = string("toy_exp/lstm_$(method)_$(info_layer)_$(seed[i]).jld")
	#save(file_name, "ntwk", ntwk)
