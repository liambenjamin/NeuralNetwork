...
using FileIO, LinearAlgebra, Random, JLD2, SparseArrays, Colors, Main.Neurons, Main.Network, Main.Partition, Main.loss, Main.penalty, Main.Helpers, Main.GRU


"""
Implements GRU Training on Toy Experiment
"""
...
function train(ntwk, loss :: Module, penalty :: Module, η :: Float64, num_epochs :: Int64, method :: String, syncFlag :: Bool, partition :: Vector{Vector{Int64}}, seed :: Int64, batch_size :: Int64, info_layer :: Int64, ntwk_type :: String)
	...
	for i=1:num_epochs
		...
		error, test_F = GRU.test_error(ntwk, info_layer, loss)
		# store epoch information
		epoch_info["F"][i] = F / n_train
		epoch_info["G"][i] = G / n_train
		epoch_info["Epoch"][i] = i
		epoch_info["Time"][i] = update_time
		epoch_info["Λ_init"][i] = norm(Λ[ntwk.features-ntwk.hid_dim+1:ntwk.features] ./ n_train)
		...
		file_name = string("toy_exp/gru/gru_$(method)_$(seed)_$(i).jld2")
...
for i=1:length(seed)
	ntwk, partition, syncFlag = GRU.specifyGRU(inp_dim, output_dim, hid_dim, fc_dim, seq_length, seed[i])

	printstyled("**************\tTraining Method: GRU $(method)\t**************\n\n", color = :yellow, bold = :true)
	...
	# save trained network to `toy_exp` directory
	#file_name = string("toy_exp/gru/gru_init_$(seed[i]).jld2")
	#save(file_name, "ntwk", ntwk)
